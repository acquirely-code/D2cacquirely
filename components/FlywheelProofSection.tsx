      ...
      <div className="mx-auto max-w-[1440px] px-5 py-2 md:py-16 sm:px-6 lg:px-20 lg:py-[8px]">
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-[minmax(0,534px)_minmax(0,621px)] lg:items-start lg:justify-center">
          <div>
                ...
                <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-5">
                  ...
                  <div className="flex-1 w-full">
                    ...
                    <p className="mt-1 text-[14px] font-semibold leading-6 text-[#7B8BA0] sm:text-[16px] sm:leading-[22px]">
                      Revenue stuck at $6K/month with ROAS 2.5x - couldn&apos;t scale without breaking performance
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-8 overflow-hidden rounded-[21px] border border-[#C9F6E1] bg-[#DEFBED]">
              <div className="grid grid-cols-[1.1fr_0.9fr_1.1fr] gap-4 border-b border-[#B7ECCD] px-5 py-4 text-[11px] font-bold uppercase tracking-[1.2px] text-[#4D6B57] sm:px-8 sm:text-[12px]">
                <span>Metric</span>
                <span>Before</span>
                <span>After 90 Days</span>
              </div>

              <div className="divide-y divide-[#B7ECCD]">
                {caseStudyResults.map((result) => (
                  <div
                    key={result.metric}
                    className="grid grid-cols-[1.1fr_0.9fr_1.1fr] gap-4 px-5 py-4 text-[14px] leading-6 text-[#1F2937] sm:px-8 sm:text-[16px]"
                  >
                    <span className="font-semibold text-[#111827]">{result.metric}</span>
                    <span className="text-[#4B5563]">{result.before}</span>
                    <span className="font-semibold text-[#047857]">{result.after}</span>
                  </div>
                ))}
              </div>
            </div>
